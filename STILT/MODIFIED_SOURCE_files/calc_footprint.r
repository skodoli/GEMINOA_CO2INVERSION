...
calc_footprint <- function(p, output = NULL, r_run_time,
 ...
 # glati <- head(seq(ymn, ymx, by = yres), -1)
  glong <- linspace(xmn,xmx,nx)
  glati <- linspace(ymn,ymx,ny)
  ...
  glong_buf <- linspace(xmn - (xbuf*xres), xmx + ((xbuf - 1)*xres), nx+xbuf*2-1)
  glati_buf <- linspace(ymn - (ybuf*yres), ymx + ((ybuf - 1)*yres), ny+ybuf*2-1)
  ...
  # Remove spatial buffer around domain used in kernel aggregation
   size <- dim(foot)
 glong_buf[(xbuf+1):(size[1]-xbuf+1)]
 glati_buf[(ybuf+1):(size[2]-ybuf+1)]
  #foot <- foot[(xbuf+1):(size[1]-xbuf), (ybuf+1):(size[2]-ybuf), ] / np
  foot <- foot[(xbuf+1):(size[1]-xbuf+1), (ybuf+1):(size[2]-ybuf+1), ] / np
