...
# User inputs ------------------------------------------------------------------
project <- 'myproject_LPDM'
stilt_wd <- file.path('/mnt/data2/skodoli', project)
...
t_start <- '2022-04-20 14:00:00'
t_end   <- '2022-04-24 14:00:00'
...
x_seq <- seq(xmn, xmx, by = xres)
y_seq <- seq(ymn, ymx, by = yres)
grid_points <- expand.grid(x = x_seq, y = y_seq)
nx <- round((xmx - xmn) / xres) + 1
ny <- round((ymx - ymn) / yres) + 1

# Meteorological data input
met_path           <- '/mnt/data2/skodoli/myproject_LPDM/hysplit_data2arl/arw2arl/arlout_wrfd02_apr22'
met_file_format    <- 'wrfout_d01_%Y-%m-%d.arl'
#met_file_format    <- 'wrfout_d0x_%Y-%m-%d_%H:%M:%S.arl'
