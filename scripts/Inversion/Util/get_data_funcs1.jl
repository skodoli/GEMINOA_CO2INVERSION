...
function get_footprint_data(lat_range,lon_range,time_range)
   ...
   path  = "/home/skodoli/INVERSION/BEACON_inv_tr/"
   key   = "obs_"
...
function get_EmsSectoral_data(lat_range,lon_range,time_range)
   # Get gridinfo from the first file
   searchdir(path,key) = filter(x->occursin(key,x), readdir(path))
   path  = "/home/skodoli/INVERSION/BEACON_inv_try/"
   key   = "MERGED_UKGHG_"
   flist = searchdir(path,key)
   nEms  = length(flist)
   # Create a vector of file names
...
function save_emissions(outDir,ems,lon,lat,emsTimes)
   ...
   for i = 1:size(ems)[3]
      ...
      priname  = @sprintf( "./MERGED_UKGHG_%s.nc", Dates.format( emsTimes[i], Dates.DateFormat( "yyyyxmmxddxHH" ) ) )
