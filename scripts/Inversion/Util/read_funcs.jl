...
### =----------------------------------------------------------------------
### = SUBFUNCTIONS:
### =  ( 1) read_lat              :: Reads the latitude data.
### =  ( 2) read_lon              :: Reads the longitude data.
### =  ( 3) read_agl              :: Reads the altitude data.
### =  ( 4) read_co2              :: Reads the CO2 data.
### =  ( 5) read_co2_err          :: Reads the CO2 uncertainty data.
### =  ( 6) read_bkg_co2          :: Reads the background CO2 data.
### =  ( 7) read_bkg_err          :: Reads the background CO2 uncertainty.
### =  ( 8) read_obs_time         :: Reads the observation time (DateTime).
### =  ( 9) read_ftime            :: Reads the footprint times (DateTime).
### =  (10) read_footprint        :: Reads the footprint.
### =  (11) read_sparse_footprint :: Reads the footprint as a sparse array.
### =  (12) read_emissions        :: Reads the emissions.
### =----------------------------------------------------------------------
### = INPUTS:
### =  (  ) N/A
### =----------------------------------------------------------------------
...
@everywhere function read_emissions_bio(emsDat)
   ...
   try
      ncid = NetCDF.open(file_name)
      out  = NetCDF.readvar(ncid,"flx_natural",start=[findfirst(lon_ind),findfirst(lat_ind)],count=[sum(lon_ind),sum(lat_ind)])
      out
   catch
      NaN
   end
